...
OUTPUT_FILENAME = "nick-kuchar_titles_and_descriptions.html"

def scrape_product_links(base_url):
    """
    Fetches multiple collection pages and extracts the URLs for each product.
    
    Args:
        base_url (str): The base URL of the collection page.

    Returns:
        list: A list of full URLs for each product.
    """
    print(f"Starting to scrape product links from multiple pages...")
    product_links = []
    
    # Loop through pages 1 to 8 as requested
    for page_num in range(1, 9):
        # Construct the URL for the current page
        page_url = f"{base_url}?page={page_num}"
        if page_num == 1:
            page_url = base_url  # Page 1 does not have the "?page=1" parameter
        ...
        try:
            ...
            # Add a small delay between each page request to be polite to the server.
            time.sleep(1)
            
        except requests.exceptions.RequestException as e:
            print(f"Error fetching page {page_num}: {e}")
            # If a page fails, we can stop the loop or just log the error and continue.
            # Here, we'll continue to try the next page.
    
    print(f"Found a total of {len(product_links)} product links across all pages.")
